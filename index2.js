...
redshiftClient.connect(function(err) {
    if (err) throw err;
        ...
        redshiftClient.query(queryString, options, function(err, data) {
            if (err) throw err;
            else {
                for (let d in data.rows) {
                    ...
                    //if we alreadt sent one to this guy dont look for another
                    // if (notificationSent.indexOf(data.rows[d].customer)) {
                    //     console.log('we already sent one to this person');
                    //     continue;
                    // }
                                    ...
                                    response.on('end', function() {
                                        ...
                                        admin.messaging().sendToDevice(registrationTokens, payload)
                                            .then(function(response) {
                                                ...
                                                //mark the customer to not send another notification 
                                                // notificationSent.push(JSON.parse(customer).id);
